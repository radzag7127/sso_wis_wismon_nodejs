...
async function simulateWorkflow() {
    console.log("Attempting to connect to MySQL database for wirahusada_wismon...");
    ...
    try {
        ...
        const [rowsAkun] = await connection.execute("SELECT id, kode_akun, nama_akun FROM akun LIMIT 5");
        console.log(rowsAkun);
        console.log("\n--- Data from Table: jenistransaksi (wirahusada_wismon) ---");
        const [rowsJenisTransaksi] = await connection.execute("SELECT id, nama_jenis FROM jenistransaksi LIMIT 5");
        console.log(rowsJenisTransaksi);
        console.log("\n--- Data from Table: transaksi (wirahusada_wismon) ---");
        const [rowsTransaksi] = await connection.execute("SELECT id, kode_transaksi, total FROM transaksi LIMIT 5");
        ...
        if (error instanceof Error) {
            console.error("Error during database workflow simulation (wirahusada_wismon):", error.message);
        }
        else {
            console.error("An unknown error occurred during database workflow simulation (wirahusada_wismon):", error);
        }
    }
    finally {
...
simulateWorkflow();
//# sourceMappingURL=wirahusada_wismon_workflow.js.map